...
{
	//original insertion sort algorithm was copied
	//swapping code was modified.
...
int main()
{
	...
	// time.
	{
		...
		vector <Student*> sptr;//vector of student pointers
		
		for (vector<Student>::iterator it = auxStuds.begin(); it != auxStuds.end(); it++)
		{
			sptr.push_back(&(*it));//adding to the student pointer vector stuff from auxstuds
		}
		
		sort(sptr.begin(), sptr.end(), compareStudentPtr);//using the STL sort algorithm

		vector<Student*>::iterator itptr = sptr.begin();
		for (vector<Student>::iterator it = studs.begin(); it != studs.end(); it++)
		{//using two iterators to go through studs and 
			//my student pointer vector to add it back to studs
			swap(*it, *(*itptr));
			itptr++;
		}
